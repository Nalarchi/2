...
void applyCannyFilter(unsigned char *image, int width, int height) {
    ...
    for (int y = 1; y < height - 1; y++) {
        for (int x = 1; x < width - 1; x++) {
            int resultIndex = (y * width + x) * 4;
            image[resultIndex] = (unsigned char) direction[y * width + x];
            image[resultIndex + 1] = (unsigned char) direction[y * width + x];
            image[resultIndex + 2] = (unsigned char) direction[y * width + x];
